(function() {
  ...
  async function runSuite() {
    ...
    try {
      ...
      // 8. Test Badge Case (Visual Collection)
      {
        console.log("Testing Badge Case functionality...");
        ...
        window.__test_helpers__.renderState(true); // Rebuild grid to reflect clean state
        
        assert(state.badgeCase.length === 0, "Badge case should be empty initially");
        assert(Object.keys(state.currentWeekBadges).length === 0, "Current week badges should be empty initially");
        ...
        // Verify temporary badge is added
        assert(state.currentWeekBadges[today] !== undefined, "Temporary badge should be added for today");
        assert(state.currentWeekBadges[today].partnerId === state.partnersData[state.partnerFamily].stageId, "Badge partner ID should match active partner");
        assert(state.badgeCase.length === 0, "Permanent badge case should still be empty");

        // Uncheck one task to make day incomplete
        const firstTask = tasks[0];
        const cbToUncheck = document.querySelector(`input[data-day="${today}"][data-task="${firstTask.id}"]`);
        if (cbToUncheck && cbToUncheck.checked) {
          cbToUncheck.click();
          await sleep(100);
        }

        // Verify temporary badge is removed
        assert(state.currentWeekBadges[today] === undefined, "Temporary badge should be removed when day is incomplete");
        ...
        assert(state.currentWeekBadges[today] !== undefined, "Temporary badge should be added again");

        // Trigger Reset Week via UI
        const resetBtn = document.getElementById('reset-btn');
        resetBtn.click();
        await sleep(100);
        const confirmYesBtn = document.getElementById('confirm-yes-btn');
        confirmYesBtn.click();
        await sleep(200); // Wait for reset to complete and state to save

        // Verify badge is committed to permanent case and temp is cleared
        assert(state.badgeCase.length === 1, "Badge should be committed to permanent case");
        assert(state.badgeCase[0].partnerId === state.partnersData[state.partnerFamily].stageId, "Committed badge partner ID should match");
        assert(Object.keys(state.currentWeekBadges).length === 0, "Current week badges should be cleared");
