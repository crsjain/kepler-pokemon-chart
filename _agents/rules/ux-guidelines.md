...
- **Visual Hierarchy & Contrast Invariant (Yellow Stars Pop First)**: Kepler's earned yellow stars (`🌟`) must always be the primary visual heroes in the daily total row. To overcome emoji gradient softness on light cells, earned stars feature a warm amber base drop shadow (`rgba(180, 83, 9, 0.45)`), golden aura (`rgba(245, 158, 11, 0.6)`), and a subtle radial halo on `.day-total-cell.unlocked`. Conversely, the unearned ghost star socket (`.future-star`) is intentionally subordinated: rendered with a soft Slate 400 stroke (`#94a3b8`, 1.8px), Slate 100 fill (`#f1f5f9`), and 0.65 opacity, ensuring the user's eye naturally locks onto earned achievements first.
