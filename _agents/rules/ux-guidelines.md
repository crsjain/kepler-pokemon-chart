...
- **Visual Hierarchy & Cohesion Invariant (Pokémon Gym Badge Vector Assets)**: 
  - **Shared Art Style**: To achieve visual harmony with the 2.5px dark charcoal-outlined Pokéballs in the grid, the daily total row employs custom vector SVG assets rather than disparate, platform-dependent system emojis.
  - **Earned Star Badge (27px × 27px, `.badge-indicator.unlocked`)**: Rendered with Pikachu Yellow fill (`#ffcb05`), a 2.6px charcoal border (`#2d3748`), faceted warm amber 3D shading (`#d97706`, 32% opacity), a specular cartoon glint at the apex, and crisp 2px retro elevation (`drop-shadow(0 2px 0 rgba(180, 83, 9, 0.35))`). Unlocked cells use a subtle warm tile background (`rgba(254, 243, 199, 0.45)`, `border-radius: 6px`) eliminating fuzzy Gaussian blur smudges. Star badges remain clean, crisp, and static to eliminate re-render jumpiness and unprompted pulsing.
  - **Row Header Star (22px × 22px, `.total-desc .task-emoji`)**: Perfectly matches the Gym Badge star geometry and stroke weight, unifying the description header with the day cells.
  - **Unearned Ghost Star Socket (27px × 27px, `.badge-indicator.future-star`)**: Forms the exact geometric recessed socket for the Gym Badge star (`#f1f5f9` fill with 2.2px `#94a3b8` stroke, 0.65 opacity). When Kepler completes a day, the socket fills in seamlessly into the Gym Badge star.
  - **Accessibility & Screen Reader Compatibility**: Vector star containers include hidden `<span class="sr-only">🌟</span>` and `<span class="sr-only">☆</span>` tags respectively to guarantee semantic parity and 100% test compatibility.
- **Future & Today In-Progress Rest Days Invariant (Ghost Star Until Midnight Rollover)**: A day where all tasks are excused (all rest passes `💤` or bonus tasks `✨` with 0 required chores) awards its free rest star strictly in arrears after the day has concluded (`dateStr < todayStr`). While the day is in the future (`dateStr > todayStr`) or in progress today (`dateStr === todayStr`), it must remain an unlit ghost star `☆` (`.future-star`). Free rest stars can never be awarded in advance, logged prematurely to Star Vault, or rendered as yellow stars before midnight rollover.
...
## 18. Landscape Split-View Chart Height Stability (Zero Historical Paging Jitter)
- **Coupled Grid Row Alignment**: In landscape tablet/desktop view (`@media (min-width: 1024px) and (orientation: landscape)`), `.chart-container` spans `grid-row: 2 / span 2` alongside Column 1 (`.trainer-card` and `.mega-milestone-card`), and `.weekly-grid` uses `height: 100%` to fill the available height.
- **Fixed Badge Status Height Guardrail**: To prevent the task chart and table rows from jumping or stretching vertically when navigating between historical and current weeks (or when weekly badges are unlocked), `.badge-status` in `.weekly-badge-card` must remain strictly locked to `height: 2.6em; min-height: 2.6em; max-height: 2.6em; overflow: hidden;`.
- **Concise Primary Status & Tooltip Requirements**: In the active/current week before a badge is unlocked, `.badge-status` must display a clean, concise call-to-action (`Clear all goals to unlock!`) that fits within 2 lines. Exhaustive task breakdowns (e.g. `Chinese, Writing: 5 days...`) must be attached to the `title` attribute for hover inspection, eliminating multi-line paragraph bloat that would otherwise push the sidebar and stretch the weekly grid.
