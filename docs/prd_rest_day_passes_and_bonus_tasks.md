...
6. **Child-Friendly Daily Totals (No Number Parsing)**: The Daily Total row displays only the visual status icons (❌ for incomplete past missed days, ☆ for today in-progress and unreached future days, 🌟 for complete days, with glowing pulse for Super Trainer bonus days, and ➖ for superseded days). Fractional task counts (e.g. `1 / 3 (+1)`, `0 / 0 ⭐`) are omitted from the visual layout so a 7-year-old child can instantly parse their progress without cognitive overload. Full counts remain accessible via hover tooltips for parents. Days with all tasks excused (rest/bonus with 0 required chores) award their free rest star 🌟 strictly in arrears once the day concludes (midnight rollover); future and in-progress rest days strictly display the ghost star ☆.
