...
- 🔒 **Parent Admin Panel**: Password-protected (`zxcv`) options dashboard in the footer:
  ...
  - **Per-Child Wipe All Progress**: `Wipe All Progress` in the Admin Danger Zone resets **only the active child** — partners, XP, levels, badges, stars, and chart history return to the start, while that child's activities, rewards, and settings are kept. Other children are never affected. The confirm names the child (`Reset Kepler`).
